...
namespace BookShop
{
    public partial class Books : Form
    {
        ...
        {
            Con.Open();
            string query = "select * from BookTbl where Категория= '"+CatCbSearchCb.SelectedItem.ToString()+"'";
            SqlDataAdapter sda = new SqlDataAdapter(query, Con);
            SqlCommandBuilder builder = new SqlCommandBuilder(sda);
            var ds = new DataSet();
            sda.Fill(ds);
        ...
        private void EditBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Con.Open();
                    string query = "update BookTbl set Название='"+BTitleTb.Text+"',Автор='"+BautTb.Text+"',Категория='"+BCatCb.SelectedItem.ToString()+"',Количество="+QtyTb.Text+",Цена="+PriceTb.Text+" where BId="+key+";";
                    SqlCommand cmd = new SqlCommand(query, Con);
                    cmd.ExecuteNonQuery();
